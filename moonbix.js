...
class Binance {
  ...
  async countdown(seconds) {
    for (let i = seconds; i > 0; i--) {
      ...
      process.stdout.write(
        `[${timestamp}] [*] Waiting ${i} seconds to continue...`
      );
  ...
  async gameData() {
    try {
      const response = await axios.post(
        "https://moonbix-server.jkl.my.id/moonbix/api/v1/play",
        this.game_response
      );
  ...
  async completeGame(accessToken) {
    try {
      ...
      if (response.data.success) {
        this.log(
          `Game completed successfully | Earned ${this.game.log} points`,
          "custom"
        );
        return true;
      }
  ...
  async playGameIfTicketsAvailable(queryString, accountIndex, firstName) {
    console.log(
      `========== Account ${accountIndex} | ${firstName.green} ==========`
    );
  ...
  async main() {
    ...
    while (true) {
      ...
      const randomCountdown =
        Math.floor(Math.random() * (1800 - 300 + 1)) + 300; // Between 5m and 30m
